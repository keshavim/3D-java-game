...
public class Shader {
    ...
    private List<String> missingUniform;
    ...
    private int getShaderType(String pattern) {
        switch (pattern.toUpperCase()) {
            ...
            default:
                throw new RuntimeException("Invalid shader type: " + pattern);
        }
    }
    ...
    private int getLocation(String name) {
        ...
        if (location < 0 && !missingUniform.contains(name)) {
            System.err.println("Could not find uniform [" + name + "] in shader program [" + id + "]");
            missingUniform.add(name);
        }
        return location;
    }
